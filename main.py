...
def normalize_df(df):
    ...
    for r in required:
        if r not in df.columns:
            df[r] = df.get("c", np.nan) if r != "v" else np.nan
    return df[required]
...
def fetch_alphavantage_crypto(symbol):
    try:
        ...
        if not timekey: return pd.DataFrame()
        ts = j[timekey]
        rows = [{"time":pd.to_datetime(t),
                 "o":float(vals.get("1a. open (USD)",0)),
                 "h":float(vals.get("2. high (USD)",0)),
                 "l":float(vals.get("3. low (USD)",0)),
                 "c":float(vals.get("4a. close (USD)",0)),
                 "v":np.nan} for t, vals in ts.items()]
...
def fetch_twelvedata(symbol):
    try:
        ...
        rows = [{"time":pd.to_datetime(v["datetime"]),
                 "o":float(v["open"]),
                 "h":float(v["high"]),
                 "l":float(v["low"]),
                 "c":float(v["close"]),
                 "v":float(v.get("volume",np.nan))} for v in reversed(j["values"])]
...
def fetch_forex_alpha(symbol):
    try:
        ...
        ts = j[key]
        rows = [{"time":pd.to_datetime(t),
                 "o":float(vals.get("1. open",0)),
                 "h":float(vals.get("2. high",0)),
                 "l":float(vals.get("3. low",0)),
                 "c":float(vals.get("4. close",0)),
                 "v":np.nan} for t, vals in ts.items()]
        return normalize_df(pd.DataFrame(rows).sort_values("time").reset_index(drop=True))
    except:
        return pd.DataFrame()
...
def synthesize_ohlcv_from_close(close_df, needed=MIN_CANDLES):
    if close_df.empty:
        ...
        closes = closes[-needed:] if len(closes)>=needed else closes + [closes[-1]]*(needed-len(closes))
...
class DataSource:
    def fetch_crypto(symbol):
        ...
        if df.shape[0] < MIN_CANDLES:
            df = fetch_alphavantage_crypto(symbol)
        if df.shape[0] < MIN_CANDLES:
            df = synthesize_ohlcv_from_close(df, MIN_CANDLES)
        return df
...
class SignalEngine:
    def generate(df):
        ...
        last = df.iloc[-1]
        direction = "BUY" if last["RSI"]<50 and last["MACD"]>0 and last["c"]>last["EMA20"] else "SELL"
        price = float(last["c"])
    ...
    async def watch(cls, asset, user_id, context):
        ...
        while True:
            ...
            if direction=="BUY" and price<sl:
                await context.bot.send_message(user_id,f"❗ {asset}: Price below SL. Last: {round(price,6)}")
                break
            if direction=="SELL" and price>sl:
                await context.bot.send_message(user_id,f"❗ {asset}: Price above SL. Last: {round(price,6)}")
                break
            if direction=="BUY" and price>tp:
                await context.bot.send_message(user_id,f"✅ {asset}: TP hit. Last: {round(price,6)}")
                break
