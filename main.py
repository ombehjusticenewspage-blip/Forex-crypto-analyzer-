...
def signal(df, news_sentiment=0, weight=1):
    ...
    s+=1.2 if news_sentiment>0.05 else -1.2 if news_sentiment<-0.05 else 0
    s*=weight
    ...
    confidence = min(max((s+5)/10*100,0),100)

    if s>=3: d="BUY"
    elif s<=-3: d="SELL"
    else: return None
    p=last["c"]; atr=last["ATR"]
    return {"dir":d,"entry":p,"sl":p-atr*1.5 if d=="BUY" else p+atr*1.5,
            "tp":p+atr*3 if d=="BUY" else p-atr*3,"atr":atr,"score":s,"confidence":round(confidence,2),"be":False}
...
async def multi_tf_signal(session,symbol):
    ...
    timeframes=[("5min",1),("15min",1.5),("1h",2),("5h",3)]
    dfs=[]
